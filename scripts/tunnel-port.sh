...
print_help() {
  cat <<'EOF'
Expose a local HTTP port publicly through ngrok or Cloudflare Tunnel.

Usage:
  tunnel-port <port>
  tunnel-port --ngrok <port>
  tunnel-port --cf [-v|--verbose] <subdomain> <port>
  tunnel-port -h|--help

Modes:
  ngrok (default)
    Runs ngrok for localhost:<port>. If TUNNEL_NGROK_DOMAIN is set, that
    reserved domain is used; otherwise ngrok assigns a temporary URL.

  --cf
    Runs cloudflared in the foreground and publishes localhost:<port> at
    https://<subdomain>.$TUNNEL_CF_DOMAIN. On first use for a subdomain, it
    creates a named Cloudflare Tunnel and DNS route. Later runs reuse them.

    Each subdomain gets its own tunnel and config file under
    ~/.cloudflared/tunnel-port/. Starting the same subdomain again stops its
    previous local cloudflared process, so the newest run takes over.

    Before first use, authenticate this machine once with:
      cloudflared tunnel login

    Cloudflare mode requires TUNNEL_CF_DOMAIN and TUNNEL_CF_NAME, configured
    through private.nix. Use -v or --verbose for cloudflared debug logging.

Examples:
  tunnel-port 3000
  tunnel-port --ngrok 8080
  tunnel-port --cf api 3000
  tunnel-port --cf --verbose preview 5173

Keep the command running while the tunnel is needed; press Ctrl-C to stop it.
EOF
}
