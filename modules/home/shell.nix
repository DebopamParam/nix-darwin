...

{
  ...
  # NOTE: Ghostty only reads ~/.config/ghostty/config (no extension), so the
  # key must be "config" — a "config.ghostty" file is silently ignored.
  home.file.".config/ghostty/config".text = ''
    macos-option-as-alt = true

    # Command has no standard terminal encoding; send zsh's kill-line key.
    keybind = cmd+backspace=text:\x15
    # Option+Backspace → delete previous word (zsh backward-kill-word).
